...
RangeWidgets::RangeWidgets() : m_VBox_Top(Gtk::ORIENTATION_VERTICAL, 0),
								...
								//Value, lower, upper, step_increment, page_increment, page size:
								// Note that the page size value only makes a difference for
	...
	m_CheckButton.signal_toggled().connect(sigc::mem_fun(*this,
		&RangeWidgets::on_checkbutton_toggled));
	...
	m_HBox_Combo.pack_start( *Gtk::manage(new Gtk::Label("Scale Value Position",0)),
		Gtk::PACK_SHRINK);
	m_HBox_Combo.pack_start(m_ComboBox_Position);
	m_ComboBox_Position.signal_changed().connect(sigc::mem_fun(*this,
		&RangeWidgets::on_combo_position));
	...
	m_adjustment_digits->signal_value_changed().connect(sigc::mem_fun(*this,
		&RangeWidgets::on_adjustment1_value_changed));
	...
	m_adjustment_pagesize->signal_value_changed().connect(sigc::mem_fun(*this,
		&RangeWidgets::on_adjustment2_value_changed));
